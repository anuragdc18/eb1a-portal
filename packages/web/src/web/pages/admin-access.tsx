...
export default function AdminAccessPage() {
  ...
  const [expandedAdmin, setExpandedAdmin] = useState<string | null>(ADMIN_USERS[0]?.id ?? null);
      ...
      >
        ...
        <div className="space-y-2">
          {CLIENTS.map((client) => {
            ...
            const ownerAdmin = !isPrivate ? DEMO_ACCOUNTS[Object.keys(DEMO_ACCOUNTS).find(k => DEMO_ACCOUNTS[k].id === client.ownerId) ?? ""] : null;
            return (
              <div
                key={client.id}
                className="flex items-center gap-3 p-3 rounded-xl"
